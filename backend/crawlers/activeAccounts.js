...
const processAccount = async (api, client, accountId) => {
  logger.info(loggerOptions, `Processing account: ${accountId}`);
  try {
    ...
    logger.info(loggerOptions, `Error processing accountId ${accountId}: ${error?.stack || error}`);
  }
};
...
const crawler = async (delayedStart) => {
  ...
  for (const chunk of chunks) {
    ...
    await Promise.all(
      chunk.map(
        (accountId) => processAccount(api, client, accountId),
      ),
    );
